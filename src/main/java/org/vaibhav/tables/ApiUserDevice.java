...
public class ApiUserDevice implements Runnable {
    long i;
    int idx = 0;
    ...
    public ApiUserDevice(String[] endpointArray, long min, long max) {
    ...
    @Override public void run() {
        ...
        while (i < max) {
            try {
                // Insert in loop here
                for (; i < max; ++i) {
                    st.executeUpdate(String.format(UtilStrings.apiUserDevice, i));
                    if (i % 1000 == 0) {
                        System.out.println("Total rows written to api_user_device --> " + i);
                    }
                }
            }
            catch (Exception e) {
                System.out.println("Exception thrown in thread (" + endpoint[idx] + "): " + ApiUserDevice.class.getName() + " --> " + e);
