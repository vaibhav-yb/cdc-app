...
public class ApiUserPartnerTransactionLog implements Runnable {
    long i;
    int idx = 0;
    ...
    public ApiUserPartnerTransactionLog(String[] endpointArray, long min, long max) {
    ...
    @Override public void run() {
        ...
        while (i < max) {
            try {
                // Insert in loop here
                for (; i < max; ++i) {
                    st.executeUpdate(String.format(UtilStrings.apiUserPartnerTransactionLog, i));
                    if (i % 1000 == 0) {
                        System.out.println("Total rows written to api_user_partner_transaction_log --> " + i);
                    }
                }
            }
            catch (Exception e) {
                System.out.println("Exception thrown in thread (" + endpoint[idx] + "): " + ApiUserPartnerTransactionLog.class.getName() + " --> " + e);
