...
public class ApiWatchList implements Runnable {
    long i;
    int idx = 0;
    ...
    public ApiWatchList(String[] endpointArray, long min, long max) {
    ...
    @Override public void run() {
        ...
        while (i < max) {
            try {
                // Insert in loop here
                for (; i < max; ++i) {
                    st.executeUpdate(String.format(UtilStrings.apiWatchList, i));
                    if (i % 1000 == 0) {
                        System.out.println("Total rows written to api_watch_list --> " + i);
                    }
                }
            }
            catch (Exception e) {
                System.out.println("Exception thrown in thread (" + endpoint[idx] + "): " + ApiWatchList.class.getName() + " --> " + e);
